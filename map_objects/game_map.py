...
class GameMap:
    ...
    def place_entities(self, room, entities, max_monsters_per_room, max_items_per_room):
        # Get a random number of monsters & items
        ...
        for i in range(number_of_monsters):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y ==y]):
                if randint(0, 100) < 80:
                    ...
                    monster = Entity(x, y, 'o', libtcod.desaturated_green, 'Orc', render_order=RenderOrder.ACTOR, blocks=True, fighter=fighter_component, ai=ai_component)
                else:
                    fighter_component = Fighter(hp=16, defense=1, power=4)
                    ai_component = BasicMonster()
                    monster = Entity(x, y, 'T', libtcod.darker_green, 'Troll', render_order=RenderOrder.ACTOR, blocks=True, fighter=fighter_component, ai=ai_component)
